<?php
/**
 * Nova Poshta Сourier Shipping Method
 *
...
if ( ! class_exists( 'Nova_Poshta_Сourier_Shipping_Method' ) ) {
	/**
	 * Class Nova_Poshta_Shipping_Method
	 */
	class Nova_Poshta_Сourier_Shipping_Method extends WC_Shipping_Method {
		...
		public function __construct( $instance_id = 0 ) {
			$this->id                 = 'shipping_nova_poshta_courier_for_woocommerce';
			$this->title              = __( 'Nova Poshta Сourier delivery', 'shipping-nova-poshta-for-woocommerce' );
			$this->method_title       = __( 'Nova Poshta Сourier delivery', 'shipping-nova-poshta-for-woocommerce' );
			$this->method_description = __( 'Nova Poshta Сourier delivery', 'shipping-nova-poshta-for-woocommerce' );
			$this->enabled            = 'yes';
			$this->supports           = [
				'shipping-zones',
				'instance-settings',
		...
		public function init_form_fields() {
			$this->instance_form_fields = [
				'title' => [
					...
					'type'    => 'text',
					'default' => __( 'Nova Poshta Сourier delivery', 'shipping-nova-poshta-for-woocommerce' ),
				],
			];
		}
		...
		public function calculate_shipping( $package = [] ) {
			...
			$nonce = filter_input( INPUT_POST, 'shipping_nova_poshta_сourier_for_woocommerce_nonce', FILTER_SANITIZE_STRING );
			if ( wp_verify_nonce( $nonce, Main::PLUGIN_SLUG . '-shipping' ) ) {
				$request_city_id = filter_input( INPUT_POST, 'shipping_nova_poshta_сourier_for_woocommerce_city', FILTER_SANITIZE_STRING );
